...
ROOT_DIR = "/mnt/TD-VAE/Elsa/Hierarchical"
...
class MovingMNIST(torch.utils.data.Dataset):
    def __init__(self, num_sequences=1000, sequence_length=20, image_size=32, digit_size=28, speed=2, digit=99):
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        logits = torch.sigmoid(self.fc3(t))
        return logits
...
# Generate data
X_train = MovingMNIST(num_sequences=5000,sequence_length=20, speed=1, digit=99)
...
vae = TDVAE_hierachical(DBlock, BeliefLSTM,PreProcess, Decoder, 1024, 1024, belief_dim=50, latent_dim_1=latent_dim, latent_dim_2=latent_dim)       # construct the model
...
#Save file
save_loss_path = f"{ROOT_DIR}/loss_per_epoch.txt"
save_state_dict_path = f"{ROOT_DIR}/checkpoint.pt"
