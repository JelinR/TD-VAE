...
class TDVAE_hierachical_forward(nn.Module):
    def __init__(self, dblock, BeliefLSTM, preprocess, decoder, input_size, processed_x_size, belief_dim=50, latent_dim_1=8, latent_dim_2=8):
        super().__init__()
        self.latent_dim_1 = latent_dim_1
        self.latent_dim_2 = latent_dim_2
    ...
    def forward(self, data):
        ...
        B, T, C, H, W = data.shape
        self.data = data.view(B, T, -1).to(device)
        self.preprocessed_data = self.preprocess(self.data)

        self.bt = self.beliefs(self.preprocessed_data)

        ###TODO Check: Plot the first instance
        # import matplotlib.pyplot as plt
        ...
        # else:
        #     nums = [-1]
        # fig.savefig(f"{save_dir}/check_{max(nums)+1}.png")
        # print(f"Plotted {max(nums)+1}")
        ###
    ...
    def train_step(self, data, optimizer, device, beta):
        ...
        #preprocess
        # B, T, C, H, W = self.data.shape
        # data = self.data.view(B, T, -1)
        # original_data = data.clone()
        # preprocessed_data = self.preprocess(data)

        #encoder
        # bt = self.beliefs(preprocessed_data)            #TODO CHECK
        #Sample z in Layer 2 at Time t2
        mu2, logvar2 = self.belief_layer2(self.bt[:, t2, :])  # shape: [batch, latent_dim_2]
        zt2_layer2 = sample_gaussian(mu2, logvar2) #term 1

        #Sample z in Layer 1 at Time t2
        mu1, logvar1 = self.belief_layer1(torch.cat((self.bt[:, t2, :], zt2_layer2), dim=-1))
        zt2_layer1 = sample_gaussian(mu1, logvar1) #term 2

        #Contatenate to get z at Time t2
        zt2 = torch.cat((zt2_layer1, zt2_layer2), dim =-1) #term3

        # mut1_layer2, logvart1_layer2 = self.belief_layer2(self.bt[:, t1, :])  # shape: [batch, latent_dim_2] #term 4


        #smoothing
        dt = torch.full((self.preprocessed_data.size(0), 1), t2 - t1, dtype=self.bt.dtype, device=self.preprocessed_data.device)
        # mu_smooth_layer2, logvar_smooth_layer2 = self.smoothing_layer2(torch.cat((bt[:, t1, :],zt2,dt], dim=-1))
        mu_smooth_layer2, logvar_smooth_layer2 = self.smoothing_layer2(torch.cat((self.bt[:, t1, :],zt2), dim=-1))
        zt1_layer2_smooth = sample_gaussian(mu_smooth_layer2, logvar_smooth_layer2) #term5

        # mut1_layer1, logvart1_layer1 = self.belief_layer1(torch.cat((self.bt[:, t1, :], zt1_layer2_smooth), dim=-1)) #term 6

        # mu_smooth_layer1, logvar_smooth_layer1 = self.smoothing_layer1(torch.cat((bt[:, t1, :],zt2, zt1_layer2_smooth, dt], dim=-1))
        mu_smooth_layer1, logvar_smooth_layer1 = self.smoothing_layer1(torch.cat((self.bt[:, t1, :],zt2, zt1_layer2_smooth), dim=-1))
        zt1_layer1_smooth = sample_gaussian(mu_smooth_layer1, logvar_smooth_layer1) #term7

        zt1 = torch.cat((zt1_layer1_smooth, zt1_layer2_smooth ), dim = -1) #term 8

        #State distributions at time t1
        mut1_layer2, logvart1_layer2 = self.belief_layer2(self.bt[:, t1, :])  # shape: [batch, latent_dim_2] #term 4
        mut1_layer1, logvart1_layer1 = self.belief_layer1(torch.cat((self.bt[:, t1, :], zt1_layer2_smooth), dim=-1)) #term 6
...
vae = TDVAE_hierachical(DBlock, BeliefLSTM,PreProcess, Decoder, 784, 784, 
                        belief_dim=belief_dim, latent_dim_1=latent_dim, latent_dim_2=latent_dim)       # construct the model
# vae = TDVAE_hierachical_forward(DBlock, BeliefLSTM,PreProcess, Decoder, 784, 784, 
