...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        x = (self.fc3(x))
        #x = torch.sigmoid(self.fc3(x))  #TODO Added
        return x
...
def tdvae_rollout(vae, x_sample, t1=10, t_end=15, device='cuda'):
    ...
    #     break
      ...
      for dt in range(1, t_end - t1):
          ...
          #probs = torch.sigmoid(logits)    #TODO Changed. Commented
          #xt2 = probs.view(1, 32, 32)
          xt2 = logits.view(1, 32, 32)
...
def plot_predictions_with_gt(x_batch, t1, xt2_list, batch_idx, save_path):
    ...
    for j, pred_frame in enumerate(xt2_list):
        col_idx = t1 + j
        axes[1, col_idx].imshow(to_numpy(pred_frame.squeeze()), cmap='gray')
